...
export async function GET(request: NextRequest) {
  try {
    ...
    if (
      ...
      where.OR = [
        { id: { in: catalogExcursionIds }, publishStatus: 'approved' },
    ...
    const catalogOverrides =
      resellerIdParam && !resellerPanel
        ? await getCatalogOverridesMap(resellerIdParam, "excursion")
        : null;
    ...
    if (excursions.length > 0) {
      const destJoins = await db.destinationExcursion.findMany({
        where: {
          excursionId: { in: excursions.map((e) => e.id) },
          active: true,
        },
