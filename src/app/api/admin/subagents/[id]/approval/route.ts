...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const subagent = await db.subagent.findUnique({ where: { id } })

    if (!subagent) {
      return NextResponse.json({ error: 'Subagente no encontrado' }, { status: 404 })
    }
    ...
    if (approvalStatus === undefined) {
      return NextResponse.json(
        { error: 'Este subagente no usa el sistema de aprobación' },
        { status: 400 },
      )
    }
    ...
    if (action === 'approve') {
      ...
      return NextResponse.json({
        success: true,
        message: `Subagente "${subagent.agencyName}" aprobado y activado`,
      })
    }
    ...
    console.error('[AdminSubagentApproval] Error:', error)
