...
import { ResellerCatalogFilters, type CatalogFilters } from './ResellerCatalogFilters'
import { ResellerCatalogItem, CatalogItemSkeleton, type CatalogItem } from './ResellerCatalogItem'
...
export default function ResellerCatalogBrowser({ canCustomizePrices, maxCatalogItems, defaultFilter }: ResellerCatalogBrowserProps) {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('¿Estás seguro de eliminar este producto de tu catálogo?')) return
  ...
  const handleSavePrice = async (
    itemId: string,
    data: { customPrice: number | null; customName: string; customDescription: string },
  ) => {
    try {
  ...
  const filteredItems = items.filter((item) => {
    ...
    const name = item.customName || (item.sourceData.name as string) || (item.sourceData.title as string) || ''
    const location = (item.sourceData.cityName as string) || (item.sourceData.region as string) || (item.sourceData.location as string) || (item.sourceData.origin as string) || ''
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-gray-50 rounded-lg border border-dashed">
          ...
          <p className="text-gray-500 text-lg font-medium">
            {items.length === 0 ? 'Tu catálogo está vacío' : 'No hay productos con esos filtros'}
          </p>
          ...
          {items.length === 0 && (
            <Button onClick={() => setAddToCatalogOpen(true)} className="mt-4 gap-2">
              <Plus className="size-4" />
              Agregar productos
            </Button>
          )}
