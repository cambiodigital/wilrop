...
function renderMarkdown(content: string): string {
  ...
  return escapedContent
    ...
    .replace(
      /\[Captura: ([^\]]+)\]/g,
      '<div class="my-4 p-4 border border-dashed border-border rounded-lg bg-muted/30 text-center text-sm text-muted-foreground" role="img" aria-label="Captura: $1">📷 [Captura: $1]</div>',
    )
...
export default function ResellerDocumentation() {
        ...
        <article>
          <div className="mb-6">
            <Badge variant="secondary" className="mb-3">
              {CATEGORY_META[selectedArticle.category]?.label ?? selectedArticle.category}
            </Badge>
            ...
            <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
              <span className="flex items-center gap-1">
                ...
                {new Date(selectedArticle.updatedAt).toLocaleDateString('es-AR', {
                  day: 'numeric',
                  month: 'long',
                  year: 'numeric',
                })}
          ...
          <div
            className="prose prose-sm max-w-none text-foreground [&_p]:text-muted-foreground [&_li]:text-muted-foreground [&_h1]:text-foreground [&_h2]:text-foreground [&_h3]:text-foreground"
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Guías y tutoriales para gestionar tu cuenta de revendedor.
        </p>
      </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filteredArticles.map((article) => (
            <Card
              ...
              role="button"
              tabIndex={0}
              className="cursor-pointer hover:shadow-md transition-shadow group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
