...
interface Props {
  ...
  filtered: TransportProvider[];
  loading: boolean;
  search: string;
  onSearchChange: (v: string) => void;
  onCreate: () => void;
  onEdit: (p: TransportProvider) => void;
  onDelete: (id: string) => void;
}

export function TransportProvidersTable({
  filtered, loading, search, onSearchChange, onCreate, onEdit, onDelete,
}: Props) {
  return (
        ...
        <Card className="flex-1">
          <CardContent className="p-4">
            <div className="relative max-w-sm">
              ...
              <Input placeholder="Buscar proveedor..." value={search} onChange={(e) => onSearchChange(e.target.value)} className="pl-10" />
            </div>
          </CardContent>
        </Card>
        <Button onClick={onCreate}><Plus className="w-4 h-4 mr-2" /> Nuevo Proveedor</Button>
      </div>
                ...
                <TableBody>
                  {filtered.length === 0 ? (
                    ...
                    <TableRow key={p.id}>
                      ...
                      <TableCell><Badge variant="secondary" className="text-xs">{vehicleTypeLabels[p.vehicleType] || p.vehicleType}</Badge></TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1">
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-primary" onClick={() => onEdit(p)}>
